...
test_fit_RMSE = sqrt(mean(angularDiff(testds.diff_phase_deg, pred_diff_phase_deg).^2));
...
apply_bias_model = true; % if false then load existing bias model
...
if apply_bias_model,
  ...
  gds.var_diff_phase_wo_bias_deg = nan(size(gds, 1), 1);
  gds.ftag2_tag_img_rot_consensus_ratio = nan(size(gds, 1), 1);
  ...
  for gds_i = 1:num_groups,
    ...
    if (length(target_idx) >= 3),
      ...
      gds.var_diff_phase_wo_bias_deg(gds_i) = gds.stdev_diff_phase_wo_bias_deg(gds_i)^2;
      ...
      gds.ftag2_rx_deg(gds_i) = angularMean(tds.ftag2_rx_deg(target_idx));
      gds.ftag2_ry_deg(gds_i) = angularMean(tds.ftag2_ry_deg(target_idx));
      gds.ftag2_rz_deg(gds_i) = angularMean(tds.ftag2_rz_deg(target_idx));
      ...
      [tag_img_rot_consensus, tag_img_rots_unique_counts] = ...
        countUniques(tds.ftag2_tag_img_rot(target_idx));
      gds.ftag2_tag_img_rot(gds_i) = tag_img_rot_consensus;
      gds.ftag2_tag_img_rot_consensus_ratio(gds_i) = ...
        max(tag_img_rots_unique_counts)/sum(tag_img_rots_unique_counts);
    end
  end
  ...
  gds.quad_diag13_px2 = (gds.quad_corner1x_px-gds.quad_corner3x_px).^2+(gds.quad_corner1y_px-gds.quad_corner3y_px).^2;
  gds.quad_diag24_px2 = (gds.quad_corner2x_px-gds.quad_corner4x_px).^2+(gds.quad_corner2y_px-gds.quad_corner4y_px).^2;
  gds.quad_diag_sqrd_ratio = gds.quad_diag13_px2 ./ gds.quad_diag24_px2;

  gds_exclude_idx = isnan(gds.mean_diff_phase_wo_bias_deg) | ...
    (gds.ftag2_tag_img_rot_consensus_ratio < 1);
...
for filter_freq = 1:NUM_FREQS,
  ...
  for feature_var_ = { ...
      'ftag2_tx_m', 'ftag2_ty_m', 'ftag2_tz_m', ...
      'ftag2_pitch_height_scale', 'ftag2_yaw_width_scale', ...
      'ftag2_roll_deg', 'ftag2_oop_deg', 'ftag2_tag_img_rot', ...
      },
    feature_var = feature_var_{1};
...
%% Compute linear regression on phase deviation
% NOTE: stdev linear model is more interpretable than var linear model
%       due to matching units in features; also stdev model fit has higher
%       R-squard than var model fit (e.g. 0.6 vs 0.4)

stdevModelSimple = strcat('stdev_diff_phase_wo_bias_deg ~ ', ...
  ' 1', ...
  ' + ftag2_tag_img_rot*tag_freq', ...
  '');

stdevModelQuad = strcat('stdev_diff_phase_wo_bias_deg ~ ', ...
  ' 1', ...
  ' + ftag2_tag_img_rot*tag_freq', ...
  ' + ftag2_tag_img_rot*ftag2_width_px', ...
  ' + ftag2_tag_img_rot*quad_diag13_px2', ...
...
%{
% Non-pruned version
stdevModelPose = strcat('stdev_diff_phase_wo_bias_deg ~ ', ...
  ' 1', ...
  ' + ftag2_tag_img_rot*tag_freq', ...
  ' + ftag2_tag_img_rot*ftag2_txy_norm', ...
  ' + ftag2_tag_img_rot*ftag2_tz_norm', ...
...
stdevFitSimpleAll = fitlm(gds, stdevModelSimple, ...
  'CategoricalVars', {'ftag2_tag_img_rot'}, 'Exclude', gds_exclude_idx);
disp(stdevFitSimpleAll);
disp(stdevFitSimpleAll.anova);

disp('STDEV FIT QUAD (DAVID):');
stdevFitQuadAll = fitlm(gds, stdevModelQuad, ...
  'CategoricalVars', {'ftag2_tag_img_rot'}, 'Exclude', gds_exclude_idx);
disp(stdevFitQuadAll);
disp(stdevFitQuadAll.anova);

disp('STDEV FIT POSE (ANQI):');
stdevFitPoseAll = fitlm(gds, stdevModelPose, ...
  'CategoricalVars', {'ftag2_tag_img_rot'}, 'Exclude', gds_exclude_idx);
disp(stdevFitPoseAll);
disp(stdevFitPoseAll.anova);

%% Perform N-fold cross-validation and report test-set results

STDEV_TEST_SET_RATIO = 0.2;
STDEV_N_FOLD_XVAL = 4;
stdevModelXval = stdevModelPose;

total_num_obs = size(gds, 1);
set_type_ids = zeros(total_num_obs, 1); % 0 = test set, 1-STDEV_N_FOLD_XVAL: xval subsets
xval_subset_size = floor(total_num_obs*(1-STDEV_TEST_SET_RATIO)/STDEV_N_FOLD_XVAL);
xval_set_ids = repmat(1:STDEV_N_FOLD_XVAL, xval_subset_size, 1);
set_type_ids(1:numel(xval_set_ids)) = xval_set_ids(:);

stdevFitTrain = cell(STDEV_N_FOLD_XVAL, 1);
best_fit_idx = 0;
best_fit_RMSE = inf;
worst_fit_idx = 0;
worst_fit_RMSE = 0;
for xval_id = 1:STDEV_N_FOLD_XVAL,
  stdevFitTrain{xval_id} = fitlm(gds, stdevModelXval, ...
    'CategoricalVars', {'ftag2_tag_img_rot'}, ...
    'Exclude', (gds_exclude_idx | (set_type_ids == xval_id) | (set_type_ids == 0)));
  gdsXval = gds(~gds_exclude_idx & (set_type_ids == xval_id), :);
  pred_stdev_diff_phase_wo_bias_deg = stdevFitTrain{xval_id}.predict(gdsXval);
  RMSE = sqrt(mean((gdsXval.stdev_diff_phase_wo_bias_deg - pred_stdev_diff_phase_wo_bias_deg).^2));
...
stdevFit = stdevFitTrain{best_fit_idx};

testds = gds(~(gds_exclude_idx | (set_type_ids ~= 0)), :);
pred_stdev_diff_phase_wo_bias_deg = stdevFit.predict(testds);
test_fit_RMSE = sqrt(mean((pred_stdev_diff_phase_wo_bias_deg - testds.stdev_diff_phase_wo_bias_deg).^2));

fprintf('===== %d-FOLD XVAL RESULTS =====\n%s\n- best xval RMSE: %.4f (id=%d)\n- worst xval RMSE: %.4f (id=%d)\n- test RMSE: %.4f\n', ...
  STDEV_N_FOLD_XVAL, stdevModelXval, best_fit_RMSE, best_fit_idx, worst_fit_RMSE, worst_fit_idx, test_fit_RMSE);
