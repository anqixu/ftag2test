...
tag_num_slices = progress_seq{1}.tag_num_slices;
tag_num_freqs = progress_seq{1}.tag_num_freqs;
tag_phase_vec_length = tag_num_slices*tag_num_freqs;
...
for seq_id = 1:num_progress_seq,
  ...
  if ~isfield(t, 'ftag2_num_tags_detected'),
    ...
    seq_ids(exp_seqcount) = seq_id; %#ok<AGROW>
    if t.ftag2_num_tags_detected > 0,
      assert(length(t.ftag2_phases_vec) == tag_phase_vec_length);
      exp_obscount = exp_obscount + tag_phase_vec_length;
    end
  end
end

seq = zeros(exp_seqcount, 63);
seqcount = 0;

obs = zeros(exp_obscount, 70);
obscount = 0;

for seq_ps_id = 1:length(seq_ids),
  seq_id = seq_ids(seq_ps_id);
  if mod(seq_ps_id, 100) == 0,
    fprintf('Processing %d/%d entries...\n', seq_ps_id, length(seq_ids));
  end
  ...
  if t.ftag2_num_tags_detected > 0,
    ...
    quad_diag13_px = corner_d13;
    quad_diag24_px = corner_d24;
    ...
    diff_payload_str_n = ...
      sum(strrep(t.ftag2_payload_str, '_', '') ~= strrep(t.tag_source(end-tag_phase_vec_length-tag_num_slices-2:end-4), '_', ''));
    ...
    ftag2_vec = [ ...
      ...
      quad_diag13_px.^2, ...
      quad_diag24_px.^2, ...
      quad_diag13_px.^2./quad_diag24_px.^2, ...
      quad_area_px2, ...
      quad_border_mindist_px, ...
      diff_payload_str_n, ...
      sqrt(t.tag_tx_m.^2+t.tag_ty_m.^2)./t.tag_width_m, ...
      t.tag_tz_m./t.tag_width_m, ...
      cosd(tag_pitch_deg), ...
      cosd(tag_yaw_deg), ...
      mod(floor((tag_roll_deg - 45)/90), 4), ...
      sqrt(t.ftag2_tx_m.^2+t.ftag2_ty_m.^2)./t.tag_width_m, ...
      t.ftag2_tz_m./t.tag_width_m, ...
      cosd(ftag2_pitch_deg), ...
      cosd(ftag2_yaw_deg), ...
      mod(floor((ftag2_roll_deg - 45)/90), 4), ...
      ];
    
    % Create individual row observations per payload phase
    phase_idx = 0;
    for slice_j = 1:tag_num_slices,
      phases_all_freqs = 0;
      for freq_k = 1:tag_num_freqs,
