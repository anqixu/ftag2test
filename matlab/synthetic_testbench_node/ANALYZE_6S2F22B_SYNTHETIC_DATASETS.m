...
trial_id = 'random';
tds = trials.(trial_id).ds(~trials.(trial_id).ds_exclude_idx, :);
roll_range_id = mod(floor((tds.ftag2_roll_deg - 45)/90), 4);
true_roll_range_id = mod(floor((tds.tag_roll_deg - 45)/90), 4);

diag1 = ((tds.quad_corner1x_px-tds.quad_corner3x_px).^2+(tds.quad_corner1y_px-tds.quad_corner3y_px).^2);
diag2 = ((tds.quad_corner2x_px-tds.quad_corner4x_px).^2+(tds.quad_corner2y_px-tds.quad_corner4y_px).^2);
diagR = diag1 ./ diag2;

fitds = dataset( ...
    {categorical(tds.id_seq), 'id_seq'}, ...
    {categorical(tds.id_frame), 'id_frame'}, ...
    {ordinal(tds.tag_slice), 'slice'}, ...
    {tds.tag_freq, 'freq'}, ...
    {tds.diff_phase_deg, 'diff_phase_deg'}, ...
    {abs(tds.diff_phase_deg), 'abs_diff_phase_deg'}, ...
    {tds.ftag2_phase_deg, 'obs_phase_deg'}, ...
    {categorical(tds.tag_slice_wide_phase_id), 'true_tag_slice_wide_phase_id'}, ...
    {tds.tag_width_m, 'true_width_m'}, ...
    {tds.tag_tx_m, 'true_tx_m'}, ...
    {tds.tag_ty_m, 'true_ty_m'}, ...
    {tds.tag_tz_m, 'true_tz_m'}, ...
    {tds.tag_pitch_deg, 'true_pitch_deg'}, ...
    {tds.tag_yaw_deg, 'true_yaw_deg'}, ...
    {tds.tag_roll_deg, 'true_roll_deg'}, ...
    {tds.tag_oop_deg, 'true_oop_deg'}, ...
    {sqrt(tds.tag_tx_m.^2+tds.tag_ty_m.^2)./tds.tag_width_m, 'true_dist_lateral_m'}, ...
    {tds.tag_tz_m./tds.tag_width_m, 'true_dist_outwards_m'}, ...
    {cosd(tds.tag_pitch_deg), 'true_pitch_height_scale'}, ...
    {cosd(tds.tag_yaw_deg), 'true_yaw_width_scale'}, ...
    {categorical(true_roll_range_id), 'true_roll_range_id'}, ...
    {tds.ftag2_width_px, 'width_px'}, ...
    {tds.ftag2_tx_m, 'tx_m'}, ...
    {tds.ftag2_ty_m, 'ty_m'}, ...
    {tds.ftag2_tz_m, 'tz_m'}, ...
    {tds.ftag2_pitch_deg, 'pitch_deg'}, ...
    {tds.ftag2_yaw_deg, 'yaw_deg'}, ...
    {tds.ftag2_roll_deg, 'roll_deg'}, ...
    {tds.ftag2_oop_deg, 'oop_deg'}, ...
    {categorical(tds.ftag2_tag_img_rot), 'tag_img_rot'}, ...
    {diag1, 'diag1'}, ...
    {diag2, 'diag2'}, ...
    {diagR, 'diagR'}, ...
    {sqrt(tds.ftag2_tx_m.^2+tds.ftag2_ty_m.^2)./tds.tag_width_m, 'dist_lateral_m'}, ...
    {tds.ftag2_tz_m./tds.tag_width_m, 'dist_outwards_m'}, ...
    {cosd(tds.ftag2_pitch_deg), 'pitch_height_scale'}, ...
    {cosd(tds.ftag2_yaw_deg), 'yaw_width_scale'}, ...
    {categorical(roll_range_id), 'roll_range_id'});

truenaivefitds = dataset( ...
    {ordinal(tds.tag_slice), 'slice'}, ...
    {tds.tag_freq, 'freq'}, ...
    {tds.diff_phase_deg, 'diff_phase_deg'}, ...
    {tds.tag_phase_deg, 'true_phase_deg'}, ...
    {categorical(tds.tag_slice_wide_phase_id), 'true_tag_slice_wide_phase_id'}, ...
    {tds.tag_width_m, 'true_width_m'}, ...
    {tds.tag_tx_m, 'true_tx_m'}, ...
    {tds.tag_ty_m, 'true_ty_m'}, ...
...
%% David's best bias fits on Wednesday

%fitTest = stepwiselm(fitds, 'ResponseVar', 'diff_phase_deg', 'PredictorVars', ...
%  {'tag_img_rot', 'freq', 'width_px', 'diag1', 'diagR'}, 'Intercept', true);
fitDavid = fitlm(fitds, 'diff_phase_deg ~ 1 + tag_img_rot*freq + tag_img_rot*width_px + tag_img_rot*diag1 + tag_img_rot*diagR + width_px*diag1 + width_px*diagR + diag1*diagR')
fitDavidDuh = fitlm(fitds, 'diff_phase_deg ~ 1 + tag_img_rot + freq + tag_img_rot*freq')

%% Anqi's best bias fits on Wednesday

fitAnqi = fitlm(fitds, 'linear', 'ResponseVar', 'diff_phase_deg', ...
    'PredictorVars', {'freq', 'dist_outwards_m', 'pitch_height_scale', 'yaw_width_scale', 'tag_img_rot'})
fitAnqiDuh = fitlm(fitds, 'linear', 'ResponseVar', 'diff_phase_deg', ...
    'PredictorVars', {'tag_img_rot'})

%% Interpretable fit of bias and variance
...
% fitPhase = fitlm(fitds, 'diff_phase_deg ~ obs_phase_deg + freq', 'Intercept', false)
% 
% fitNaivePose = fitlm(fitds, ...
%     'diff_phase_deg ~ true_tx_m + true_ty_m + true_tz_m + true_pitch_deg + true_yaw_deg + true_roll_deg + true_oop_deg + freq', 'Intercept', false)
% 
% fitNaivePoseNoisy = fitlm(fitds, ...
%     'diff_phase_deg ~ tx_m + ty_m + tz_m + pitch_deg + yaw_deg + roll_deg + oop_deg + tag_img_rot + freq', 'Intercept', false)
% 
% fitInterpretedNoisy = fitlm(fitds, 'diff_phase_deg ~ dist_lateral_m + dist_outwards_m + pitch_height_scale + yaw_width_scale + roll_range_id + freq', 'Intercept', false)
% 
% fitInterpretedNoisy = fitlm(fitds, 'diff_phase_deg ~ dist_lateral_m + dist_outwards_m + pitch_height_scale + yaw_width_scale + roll_range_id + freq', 'Intercept', false)
