...
all_trials_file = fullfile(dataset_dir, 'trials', '6s4f2111b_all_trials.processed.mat');
...
if generate_tag_images,
  GENERATE_6S4F2111B_DATASETS; %#ok<*UNRCH>
...
if synthesize_and_decode_scenes,
  ...
  SYNTHESIZE_6S4F2111B_MANUAL_DATASET;
  SYNTHESIZE_6S4F2111B_RANDOM_DATASET;
...
if process_raw_trials,
  ...
  for i = 1:length(trial_ids),
    ind_trial_file = sprintf('%s/trials/6s4f2111b_%s.mat', dataset_dir, trial_ids{i});
    fprintf('\nPROCESSING %s...\n\n', ind_trial_file);
    load(ind_trial_file);
