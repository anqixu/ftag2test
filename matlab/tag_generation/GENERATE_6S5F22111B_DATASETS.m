...
%% Specify payloads for manually-sampled tags for 6S5F22111B family
% Since 2^6 bits is too large, even when divided by 6S, we must resort
...
%% Specify payloads for random-sampled tags for 6S5F22111B family
...
while curr_random_tag_idx <= num_random_tags,
  payload_bits = uint8(round(rand(6, 6)));
...
manual_dataset_dir = sprintf('%s/6S5F22111B_manual_set/', dataset_dir);
...
for idx = 1:length(manual_set_payload_bits),
  [tag, phase_ids] = genFTag2Marker6S5F22111B(manual_set_payload_bits{idx}, tag_width_px);
  tagPadded = padTagBorder(tag, tag_with_border_width_px);
  imwrite(tagPadded, genTagFilename(manual_dataset_dir, 'ftag2_6s5f22111b', phase_ids));
end

%% Generate randomly-sampled tag images
random_dataset_dir = sprintf('%s/6S5F22111B_random_set/', dataset_dir);
...
for idx = 1:length(random_set_payload_bits),
  [tag, phase_ids] = genFTag2Marker6S5F22111B(random_set_payload_bits{idx}, tag_width_px);
  tagPadded = padTagBorder(tag, tag_with_border_width_px);
  imwrite(tagPadded, genTagFilename(random_dataset_dir, 'ftag2_6s5f22111b', phase_ids));
end
