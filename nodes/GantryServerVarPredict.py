...
#     print '\r\nMoving = ', moved
  ...
  def imageTimeoutCB(self):
    ...
    moving = self.MOVING
    self.mutex_moving.release()
    if self.fsm == State.WAIT_SHOWING_TAGS and not moving:
...
#         print 'Image timed out'  
      ...
      state_msg.image_count_in_pos = self.num_detections
...
#       mutex_new_tag.release()
  ...
  def processIm(self,msg):    
    ...
    moving = self.MOVING
    self.mutex_moving.release()
    if self.fsm == State.WAIT_SHOWING_TAGS and self.alive and not moving:
      state_msg = ControllerState()
      ...
      state_msg.image_count_in_pos = self.num_detections
      state_msg.pos_count = self.num_positions 
      state_msg.tag_family = families[4]
      state_msg.tag_payload = self.last_payload
      state_msg.num_tag_family = 4
      self.state_pub.publish(state_msg)
      self.image_pub.publish(msg)      
      print '\rNum im: ', self.num_detections
...
#         rospy.sleep(0.1)
      ...
      elif self.fsm == State.WAIT_SHOWING_TAGS:
